...
portCreate ∷ (RGLFW t m, MonadTrace sig m) ⇒ Dynamic t GL.Window → ESettings t → m (Dynamic t (Maybe (Port f)))
...
makeDrawable ∷ (HasCallStack, MonadTrace sig m) ⇒ ObjectStream → IdToken → Di Double → m Drawable
...
disposeDrawable ∷ (HasCallStack, MonadTrace sig m) ⇒ ObjectStream → Drawable → m ()
...
newId ∷ (HasCallStack, MonadTrace sig m) ⇒ T.Text → m IdToken
newId desc = do
...
blankIdToken'setup ∷ MonadTrace sig m ⇒ m ()
...
portEnsureVisual ∷ (HasCallStack, MonadTrace sig m, PortVisual f, Typeable a, c a)
  ⇒ Port f
  → Di Double
  → Proxy (c ∷ Type → Constraint)
  → IdToken
...
portGarbageCollectVisuals ∷ ∀ sig m f a. (MonadTrace sig m, PortVisual f) ⇒ Port f → IntMap.IntMap a → m ()
