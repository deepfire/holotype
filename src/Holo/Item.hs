...
iNewToken ∷ (MonadTrace sig m, Typeable a) ⇒ Proxy a → m IdToken
iNewToken p = Port.newId $ showT $ typeRep p

_iCompToken ∷ MonadTrace sig m ⇒ Item c p → m IdToken
_iCompToken = \case
...
iSizeRequest ∷ ∀ sig c m. (MonadTrace sig m, Typeable c) ⇒ VPort → Item c PBlank → m (Item c PLayout)
...
iMandateVisual ∷ (HasCallStack, MonadTrace sig m) ⇒ VPort → Item c PLayout → [Item c PVisual] → m (Item c PVisual)
...
iRender ∷ (MonadTrace sig m) ⇒ VPort → Item c PVisual → m ()
