...
class Mutable a where
  ...
  mutate        ∷ MonadW i sig t m ⇒ Proxy i → a → Event t Ev → m (Dynamic t a)
...
class (Typeable a) ⇒ Widgety (i ∷ Type) (a ∷ Type) where
  dynWidget'    ∷ (MonadW i sig t m, Typeable a, Named a, Widgety i a, HasCallStack)
                ⇒ AElt → IdToken → Vocab i (Present i) → Dynamic t a → m (Widget i a)
  widget        ∷ (MonadW i sig t m, Typeable a, HasCallStack)
                ⇒ AElt           → Vocab i (Present i) →           a → m (Widget i a)
  --
  default dynWidget'
                ∷ (MonadW i sig t m, Mutable a, Named a)
                ⇒ AElt → IdToken → Vocab i (Present i) → Dynamic t a → m (Widget i a)
...
class (Typeable a) ⇒ Present (i ∷ Type) (a ∷ Type) where
  ...
  present       ∷ (MonadW i sig t m, HasCallStack)
                ⇒ AElt          → Vocab i (Present i) →           a → m (Widget i a)
  --
